...
public class Manager extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        navigationView = findViewById(R.id.manager_navigationView);
        ...
        fragmentTransaction.add(R.id.manager_fragment_container, new FragmentQuantities(), "FragmentQuantities"); //Create and add our first fragment into our fragment container
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) { //is called when an item in the menu is clicked on
        ...
        switch (item.getItemId()) { //tests to see which item in the drawer was clicked on in order to execute the required code
            case R.id.item_quantities:
                fragmentTransaction = getSupportFragmentManager().beginTransaction();
                fragmentTransaction.replace(R.id.manager_fragment_container, new FragmentQuantities(), "FragmentQuantities");
                fragmentTransaction.commit();
                break;
            case R.id.set_logs:
                fragmentTransaction = getSupportFragmentManager().beginTransaction();
                fragmentTransaction.replace(R.id.manager_fragment_container, new ManagerFragmentSetLog(), "FragmentSetLog");
                fragmentTransaction.commit();
                break;
        }
        return true;
    ...
    public void loadFragmentSetLog() {
        fragmentTransaction = getSupportFragmentManager().beginTransaction();
        fragmentTransaction.replace(R.id.manager_fragment_container, new ManagerFragmentSetLog(), "FragmentSetLog");
        fragmentTransaction.commit();
    }
    ...
    public void loadFragmentLogDetails(ItemLog log) {
        fragmentTransaction = getSupportFragmentManager().beginTransaction();
        fragmentTransaction.replace(R.id.manager_fragment_container, new FragmentLogDetails(log), "FragmentLogDetails");
        fragmentTransaction.commit();
    }
}